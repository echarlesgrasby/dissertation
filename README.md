# dissertation
An evolving repository of my doctoral dissertation on information quality, language engineering, and electricity market monitoring.

The following chapters are listed below: 

1. Introduction
2. Literature Review
3. Appendices
   * IRB Certificates
   * Search Queries For Literature Review
   * Schedule of Dissertation Milestones
   * Public Market Data Sources

## "License"

Cite with proper credit using one of the two following formats: 

__Plaintext__: 

Eric C. Grasby. (2027). *A Domain-Specific Language Approach to Support Monitoring and Surveillance Operations Within Wholesale Electricity Markets*. Retrieved from [https://github.com/echarlesgrasby/dissertation](https://github.com/echarlesgrasby/dissertation).

__BibTex__:

```
@misc{grasby2027system,
  author       = {Eric C. Grasby},
  title        = {A Domain-Specific Language Approach to Support Monitoring and Surveillance Operations Within Wholesale Electricity Markets},
  year         = {2027},
  howpublished = {\url{https://github.com/echarlesgrasby/dissertation}}
}
```
